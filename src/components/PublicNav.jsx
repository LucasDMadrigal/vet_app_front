...
const PublicNav = () => {
  ...
  const links = [
    {
      ...
      to: "/",
      img: "/assets/home.png",
      activeIcon: activePage.Home,
    },
    {
      text: "Register",
      to: "/Register",
      img: "/assets/register.png",
      activeIcon: activePage.Register,
    },
    {
      text: "Login",
      to: "/Login",
      img: "/assets/login.png",
      activeIcon: activePage.Login,
    },
  ];
  ...
  console.log("🚀 ~ handleLogout ~ text:", text)
	console.log("Logout");
  ...
  return (
    <>
      <header className="bg-[#8BA8C4] gap-8 py-5 flex justify-between items-center px-4 md:px-10 lg:px-20 h-[10vh]">
        <div className="flex items-center">
          <picture className="flex gap-2 lg:gap-3 items-center">
            <img
              ...
              src="/assets/icono-VetNova.png"
              alt="logo"
            />
            <h1 className="text-white font-bold text-1xl md:text-4xl lg:text-5x1">
              VetNova
            </h1>
          </picture>
        </div>
        <div className="lg:hidden">
          ...
          >
            <img
              className="w-6 h-6"
              src={
                menuOpen ? "/assets/icon-Xmenu.png" : "/assets/icon-menu.png"
              }
        ...
        <nav className="hidden lg:flex lg:items-center mt-[20px] p-2">
          <ul className="flex justify-around items-center space-x-3 gap-4">
            {links.map((anchor, id) => (
              <li key={id} className={`text-center ${anchor.text}`}>
                <Link
                  to={anchor.to}
                  className={`flex items-center text-2xl font-medium 
                      ...
                      isActive(anchor.to)
                        ...
                        : "text-white hover:text-[#FAE7D5]"
                    } 
										rounded-md p-2 transition-colors`}
                  onClick={(e) => {
                    if (anchor.text === "Logout") {
                      e.preventDefault(); // Evita que el Link haga una navegación innecesaria
                      handleLogout(anchor.text); // Llama la función de logout
                    }
                  }}
                >
                  <img
      ...
      {menuOpen && (
        ...
        >
          <nav
            className="bg-[#8BA8C4] w-3/4 max-w-xs h-full shadow-lg p-4 transition-transform transform translate-x-0 bg-opacity-90"
          ...
          >
            ...
            <ul className="flex flex-col items-center gap-4 text-2xl">
              {links.map((anchor, id) => (
                <li key={id} className={`text-center w-full ${anchor.text}`}>
                  <Link
                    to={anchor.to}

...
const MainLogin = () => {
  ...
  useEffect(() => {
    ...
    if (storedUser && storedUser.token) {
      ...
      navigate(storedUser.rol === "ROLE_ADMIN" ? "/auth/admin" : "/auth/account");
    }
  }, [dispatch, navigate]);
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/api-veterinary/login",
        user
      );
  ...
  return (
    <div className="flex flex-col md:flex-row h-[100vh] lg:h-[100vh] md:h-[100vh] ">
      <article className="text-center md:w-1/2 h-full flex flex-col justify-center">
        <h2 className="font-bold text-3xl my-10 m lg:mt-4 lg:mb-8">
        ...
        <form className="flex flex-col items-center gap-8">
          <label className="flex w-8/12 lg:w-6/12">
            <input
              ...
              required
              placeholder="EMAIL"
              className="shadow mx-2 p-4 rounded-md w-full"
            />
          </label>
          <label className="flex w-8/12 lg:w-6/12 items-center gap-2 relative">
            <input
              ...
              className="shadow mx-2 p-4 rounded-md w-full id_password"
            />
          </label>
          ...
          <button
            type="button"
            className="rounded-md max-h-14 font-bold py-2 px-4 bg-[#8BA8C4] m-2 h text-lg text-white cursor-pointer transition duration-200 ease-in-out hover:bg-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-700 focus-visible:ring-offset-2 active:scale-95 my-4"
            onClick={handleLogin}
          >
            LOG IN
          </button>
      ...
      <article className="article-bg md:w-1/2 flex flex-col justify-center items-center ">
        <div className="article-content text-white flex flex-col gap-14 justify-center items-center">
          ...
          <p className="text-gray-700 text-center text-lg">
            Discover the benefits of being part of VetNova
          </p>
          <p className="text-gray-700 text-center text-lg p-4">
            Haven't an account?
            <Link
              className="text-blue-800 font-bold italic ml-4"
              to={"/Register"}
            >
              Sign up here
            </Link>
